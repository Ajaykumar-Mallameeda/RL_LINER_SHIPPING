...
| DEV-11 | Paper does not specify the penalty rate Y_d for rejected demand (Eq. 30) | Y_d value must be sourced from LINERLIB documentation or paper's full text | Missing from extracted text | Affects profit calculation; critical for numerical reproduction | PENDING VERIFICATION |
